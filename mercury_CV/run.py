...
# Specify the parameters unique for each task
global_params = {}
global_params['n_jobs'] = -1 # number of cores to use on *your* machine
global_params['n_folds'] = 11 # number of cross-validation splits
global_params['cheating'] = True # choose between dataset specific
                                 # algoriths or general AutoML learning
ACTIVE_DATASETS = 'cjmps' # first letters of the datasets that you want
...
if os.path.isdir(codalab_run_dir): 
    ...
    global_params['n_jobs'] = -1 # Take as many cores as they have
    ...
    for basename in datanames: # Loop over datasets
        ...
        if basename=='christine':
            ...
            if running_on_codalab or (basename[0] in ACTIVE_DATASETS):
                #Y_valid, Y_test = learning_routine.process_christine(D.data['X_train'], D.data['Y_train'], D.data['X_valid'], D.data['X_test'], global_params)
                Y_valid, Y_test = learning_routine.process_my_christine(D.data['X_train'], D.data['Y_train'], D.data['X_valid'], D.data['X_test'], global_params)
            else:
                Y_valid, Y_test = [[], []]
            ...
            if running_on_codalab or (basename[0] in ACTIVE_DATASETS):
                #Y_valid, Y_test = learning_routine.process_jasmine(D.data['X_train'], D.data['Y_train'], D.data['X_valid'], D.data['X_test'], global_params)
                Y_valid, Y_test = learning_routine.default_prediction(D.data['X_train'], D.data['Y_train'], D.data['X_valid'], D.data['X_test'], global_params)
            else:
                Y_valid, Y_test = [[], []]
            ...
            if running_on_codalab or (basename[0] in ACTIVE_DATASETS):
                Y_valid, Y_test = learning_routine.process_madeline(D.data['X_train'], D.data['Y_train'], D.data['X_valid'], D.data['X_test'], global_params)    
            else:
                Y_valid, Y_test = [[], []]
            ...
            if running_on_codalab or (basename[0] in ACTIVE_DATASETS):
                Y_valid, Y_test = learning_routine.process_philippine(D.data['X_train'], D.data['Y_train'], D.data['X_valid'], D.data['X_test'], global_params)    
            else:
                Y_valid, Y_test = [[], []]
            ...
            if running_on_codalab or (basename[0] in ACTIVE_DATASETS):
                Y_valid, Y_test = learning_routine.process_sylvine(D.data['X_train'], D.data['Y_train'], D.data['X_valid'], D.data['X_test'], global_params)    
            else:
                Y_valid, Y_test = [[], []]
            
        else:
            print("============ ERROR UNKNWN KEY!!!!: {}".format(basename))
            #print 1/ 0 
            Y_valid, Y_test = learning_routine.default_prediction(D.data['X_train'], D.data['Y_train'], D.data['X_valid'], D.data['X_test'])
