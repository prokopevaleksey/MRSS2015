...
if __name__=="__main__" and debug_mode<4:	
    ...
    for basename in datanames: # Loop over datasets
        ...
        # Normalize the data
        X_train = preprocessing.scale(D.data['X_train'], axis=1, with_mean=True, with_std=True, copy=True)
        X_valid = preprocessing.scale(D.data['X_valid'], axis=1, with_mean=True, with_std=True, copy=True)
        X_test = preprocessing.scale(D.data['X_test'], axis=1, with_mean=True, with_std=True, copy=True)
        ...
        while  cycle <= 1: #max_cycle:
            ...
            #M = RForestClass(n_estimators, random_state=1, n_jobs=THREADS_NUM).fit(pca.transform(X_train), D.data['Y_train'])
            M = RForestClass(n_estimators, random_state=1, n_jobs=THREADS_NUM).fit(X_train, D.data['Y_train'])
      
            vprint( verbose,  "[+] Fitting success, time spent so far %5.2f sec" % (time.time() - start))
            
            # Make predictions
            if task == 'binary.classification':
                #Y_valid = M.predict_proba(pca.transform(X_valid))[:, 1]
                #Y_test =  M.predict_proba(pca.transform(X_test))[:, 1]
                Y_valid = M.predict_proba(X_valid)[:, 1]
                Y_test =  M.predict_proba(X_test)[:, 1]
                print Y_valid, Y_test
            elif task == 'multiclass.classification':
                #Y_valid = np.array([M.predict_proba(pca.transform(X_valid))[:, i] for i in range(K)]).T 
                #Y_test =  np.array([M.predict_proba(pca.transform(X_test))[:, i] for i in range(K)]).T 
