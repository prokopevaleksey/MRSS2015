...
if __name__=="__main__" and debug_mode<4:	
    ...
    for basename in datanames: # Loop over datasets
        ...
        time_budget = 2 * HALF_TIME_BUDGET
        ...
        while  cycle <= 1: #max_cycle:
            ...
            n_estimators = 50
            if cycle==1:
                n_estimators = 5 * int((np.floor(time_budget / time_spent_last) - 1 ) * 5) # * 5 == aim to use 5/10 of the time budget
                if n_estimators <= 0:
                    break
            ...
            if cycle == 1 and not sparse:
                ...
                # Take 10% of the most important features
                #fis = sorted(fi)
                ...
                ## Take features that have a certain cumulitive influence
                fir = sorted(fi, reverse=True)
                
                which = np.cumsum(fir) < 0.53 # cumulitive importance
                
                if ((D.info['feat_num'] > 200) and (D.info['feat_num'] < 1200)):        
            ...
            if task == 'binary.classification' or task == 'multiclass.classification':
                if sparse:
                    ...
                    M = RForestClass(n_estimators, random_state=1, n_jobs=4).fit(D.data['X_train'], D.data['Y_train'])
