...
def process_christine(X_train, Y_train, X_valid, X_test, params):
    ...
    ytestrf = modelrf.predict_proba(X_test[:, which])[:, 1]
    yvalrf = modelrf.predict_proba(X_valid[:, which])[:, 1]
             
    modelknn = KNeighborsClassifier(n_neighbors = 6, weights = 'distance', metric = 'braycurtis')
    modelknn.fit(X_train[:, which], Y_train)

    print 'KNN DONE'
    print (time.time() - t0) / 60.
             
    ytestknn = modelknn.predict_proba(X_test[:, which])[:, 1]
    yvalknn = modelknn.predict_proba(X_valid[:, which])[:, 1]
...
def process_my_christine(X_train, Y_train, X_valid, X_test, params):
    ...
    ytestrf = modelrf.predict_proba(X_test)[:, 1]
    yvalrf = modelrf.predict_proba(X_valid)[:, 1]
...
def process_jasmine(X_train, Y_train, X_valid, X_test, params):
    ...
    # Scale
    X_train_norm = preprocessing.scale(X_train[:,which], axis=1, with_mean=True, with_std=True, copy=True)
    X_valid_norm = preprocessing.scale(X_valid[:,which], axis=1, with_mean=True, with_std=True, copy=True)
    X_test_norm = preprocessing.scale(X_test[:,which], axis=1, with_mean=True, with_std=True, copy=True)
    
    # SVM
    modelsvm = svm.SVC(probability=True)
    modelsvm.fit(X_train_norm, Y_train)

    pr_svm_train = modelsvm.predict_proba(X_train_norm)[:, 1]
    pr_svm_val = modelsvm.predict_proba(X_valid_norm)[:, 1]
    pr_svm_test = modelsvm.predict_proba(X_test_norm)[:, 1]
    
    ## NB
    #modelbnb = BernoulliNB()
    #modelbnb.fit(X_train_norm, Y_train)
    #pr_bnb_train = modelbnb.predict_proba(X_train_norm)[:, 1]
    #pr_bnb_val = modelbnb.predict_proba(X_valid_norm)[:, 1]
    #pr_bnb_test = modelbnb.predict_proba(X_test_norm)[:, 1]
...
def process_sylvine(X_train, Y_train, X_valid, X_test, params):
 ...
 False, False, True, True, False, False, False, True])
    ...
    Xnewtrain = np.array(X_train[:, which])
    Xnewtest = np.array(X_test[:, which])
    Xnewval = np.array(X_valid[:, which])
    ...
    modelrf.fit(Xnewtrain, Y_train)
    
    print (time.time() - t0) / 60.

    pr_rf_train = np.round(modelrf.predict_proba(Xnewtrain)[:, 1])
    pr_rf_val = np.round(modelrf.predict_proba(Xnewval)[:, 1])
    pr_rf_test = np.round(modelrf.predict_proba(Xnewtest)[:, 1])
...
def process_philippine(X_train, Y_train, X_valid, X_test, params):
    ...
    pr_stack_train = np.vstack((pr_rf_train, pr_svm_train, pr_knn_train)).T
    pr_stack_val = np.vstack((pr_rf_val, pr_svm_val, pr_knn_val)).T
    pr_stack_test = np.vstack((pr_rf_test, pr_svm_test, pr_knn_test)).T
