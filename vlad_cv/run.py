...
verbose = False # outputs messages to stdout and stderr for debug purposes
...
default_input_dir = "/home/vlad/Microsoft_school/Hackathon_second_day/data"
...
#                    break
            ...
            if cycle > 0:
                if cycle == 1:
                    ...
                    fis = sorted(fi)
                #th = fis[- D.info['feat_num'] / 10]# we can choose it with brain!
                    
                
                th = fis[-D.info['feat_num']*(100-cycle*7)/100 - 1]
                
                #print th
                which = fi > th
                #print which
                nD.data['X_train'] = D.data['X_train'][:, which]
                nD.data['X_test']= D.data['X_test'][:, which]
                nD.data['X_valid']= D.data['X_valid'][:, which]
                print nD.data['X_train'].shape[1]
                print "selected"
            else:
                nD = copy.deepcopy(D)
...
#                vprint( verbose,  "[+] SVM croos_val done! Mean = %0.2f" % (scores.mean()))
            ...
            clf = RForestClass(n_estimators, random_state=1, n_jobs=4, min_samples_leaf = 3 , min_samples_split = 6)
#            crass_validation.
#            scores = cross_validation.cross_val_score(clf, D.data['X_train'], D.data['Y_train'], cv=5, scoring='bac_metric')
#            vprint( verbose,  "[+] RF croos_val done! Mean = %0.2f" % (scores.mean()))
#            
            kfold = cross_validation.KFold(len(nD.data['X_train']), n_folds=11)
            
            cross_vals = []
            for train, test in kfold:
                #print "dims"               
                XX = clf.fit(nD.data['X_train'][train], D.data['Y_train'][train]).predict_proba(nD.data['X_train'][test])[:,1]
                ...
                #cross_vals.append(libscores.bac_metric
#                (clf.fit(D.data['X_train'][train], D.data['Y_train'][train])
#                .predict_proba(D.data['X_train'][test])[:,1],D.data['Y_train'][test]))
                
            
                
#            cross_vals = [libscores.bac_metric(clf.fit(D.data['X_train'][train],
#                                                       D.data['Y_train'][train])
#                                                       .predict_proba(D.data['X_train'][test])
#                                                       ,D.data['Y_train'][test]) for train, test in kfold]
            score_mean = np.mean(cross_vals)
            if cycle == 0:
                print(   "")
                print(   "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")            
                print(   "")
                print(   "[+] RF croos_val w/o fs done! Mean = %0.2f" % (score_mean))                
            if cycle > 0:
                print(   "[+] RF croos_val with fs done (%i features)! Mean = %0.2f" % (100-cycle*11,score_mean))                                
            if cycle == 8:
                print(  "")
                print(   "$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")            
            ...
            if cycle == 0:    
                M = clf.fit(D.data['X_train'], D.data['Y_train'])
            ...
            if best_cycle > 0:
                                #th = fis[- D.info['feat_num'] / 10]# we can choose it with brain!
                th = fis[-D.info['feat_num']*(100-best_cycle*7 )/100 - 1]
                #print th
                which = fi > th
                #print which
            ...
            clf = RForestClass(333, random_state=1, n_jobs=-1, min_samples_leaf = 1 , min_samples_split = 3)
            M = clf.fit(D.data['X_train'], D.data['Y_train'])
                #print nD.data['X_train'].shape[1]
                #print "selected"
